...
export default function NatureBackground() {
  return (
    <div className="fixed inset-0 -z-10 bg-night-gradient overflow-hidden">
      ...
      {/* Drifting leaves - simplified circles for demo */}
      <div className="absolute bottom-0 left-10 w-6 h-6 bg-leaf-green rounded-full opacity-70 animate-leaf-drift" />
      <div className="absolute bottom-10 right-20 w-8 h-8 bg-leaf-green rounded-full opacity-60 animate-leaf-drift" style={{ animationDelay: '3s' }} />
      <div className="absolute bottom-5 left-52 w-5 h-5 bg-leaf-green rounded-full opacity-50 animate-leaf-drift" style={{ animationDelay: '5s' }} />
    </div>
  );
}
